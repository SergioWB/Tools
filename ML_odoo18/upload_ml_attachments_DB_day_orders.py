...
def get_odoo_credentials(environment="test_18"):
...
def search_sale_attachments(order_id):
    """ Busca si ya existen adjuntos para esta orden de venta en el modelo custom. """
    try:
        # Hacemos un count directo al modelo custom usando so_id
        attatchments_number = models.execute_kw(ODOO_DB_NAME, uid, ODOO_PASSWORD,
                                                'sale.order.attachment', 'search_count',
                                                [[['so_id', '=', order_id]]])

        if attatchments_number == 0:
            return 'NO ATTACHMENTS'
        else:
            return 'THERE ARE ATTACHMENTS'

    except Exception as e:
        logging.error(f'Error en search_sale_attachments: {str(e)}')
        return 'ERROR'

def upload_attachment(so_name, order_id):
    """ Adjunta la etiqueta de envío en sale.order.attachment. """
    path_attachment = f'{labels_path}/Etiqueta_{so_name}/Etiqueta de envio.txt'
...
def delete_attachments_with_name_contains(order_id, keyword='empty'):
    """ Elimina adjuntos custom de la SO cuyo nombre contenga la palabra clave. """
    try:
        attachment_ids = models.execute_kw(ODOO_DB_NAME, uid, ODOO_PASSWORD,
                                           'sale.order.attachment', 'search',
                                           [[['so_id', '=', order_id]]])
        ...
        attachments = models.execute_kw(ODOO_DB_NAME, uid, ODOO_PASSWORD,
                                        'sale.order.attachment', 'read',
                                        [attachment_ids],
                                        {'fields': ['id', 'file_name']})

        # Usamos file_name porque así se llama en tu modelo
        to_delete_ids = [att['id'] for att in attachments if
                         att.get('file_name') and keyword.lower() in att['file_name'].lower()]

        if not to_delete_ids:
            return False

        models.execute_kw(ODOO_DB_NAME, uid, ODOO_PASSWORD,
                          'sale.order.attachment', 'unlink',
                          [to_delete_ids])
        return True
...
def insert_log_message_so(order_id, so_name):
    """ Inserta el mensaje directo en el chatter de la SO """
...
def procces_db_orders(orders, local):
    ...
    limit_hour = cdmx_time.replace(hour=6, minute=0, second=0, microsecond=0)  # 2pm CDMX
    ...
    for order in orders:
        ...
        if ml_crawl_status == 'Envíos de hoy' or get_label_for_tomorrow:
            ...
            if ('Error' not in message_response) and ('Advertencia' not in message_response):
                empty_file = delete_attachments_with_name_contains(order_id, 'empty')
                ...
                #Subimos la guía usando order_id
                upload_attachment(so_name, order_id)
                #Insertamos el tracking ref
                carrier_traking_response = insert_carrier_tracking_ref_odoo_backup(order_id, so_name, carrier_tracking_ref)
                #Mandamos mensaje al chatter de la SO
                insert_log_message_so(order_id, so_name)
                ...
                if "Flex" in carrier_tracking_ref:
                    ...
                    logging.info(
                        f'DB: {message_for_tomorrow}{meessage_empty_file}. Se ha agregado la guia a la orden {so_name}. {carrier_traking_response}. FLEX: {carrier_option_response}')
                else:
                    logging.info(
                        f'DB: {message_for_tomorrow}{meessage_empty_file}. Se ha agregado la guia a la orden {so_name}. {carrier_traking_response}')
            else:
...
def procces_new_orders(orders, local):
    ...
    for order in orders:
        ...
        # Llamamos a la nueva función pasando solo el order_id
        ...
        if ml_crawl_status == 'Envíos de hoy' or get_label_for_tomorrow:
            if are_there_attachments == 'NO ATTACHMENTS':
                ...
                if ('Error' not in message_response) and ('Advertencia' not in message_response):
                    # 1. Subimos la guía usando order_id
                    upload_attachment(so_name, order_id)
                    # 2. Insertamos el tracking
                    carrier_traking_response = insert_carrier_tracking_ref_odoo_backup(order_id, so_name,
                                                                                       carrier_tracking_ref)
                    # 3. Mandamos mensaje a la SO
                    insert_log_message_so(order_id, so_name)
                    ...
                    if "Flex" in carrier_tracking_ref:
                        carrier_option_response = insert_LOIN_carrier_odoo(order_id, so_name)
                        logging.info(
                            f'{message_for_tomorrow}Se ha agregado la guia a la orden {so_name}. {carrier_traking_response}. FLEX: {carrier_option_response}')
                        process_coun += 1
                    else:
                        logging.info(
                            f'{message_for_tomorrow}Se ha agregado la guia a la orden {so_name}. {carrier_traking_response}')
                        process_coun += 1
                else:
                ...
                logging.info(f'La orden {so_name} YA tiene guia adjunta, no se consulta ML ni se agrega guia.')
            else:
